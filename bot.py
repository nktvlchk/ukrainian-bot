...
async def google_translate(text: str, direction: str) -> str:
    """Translate text using Google Translate. direction: 'ru_uk' or 'uk_ru'.
...
async def handle_word(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── PHRASES (multiple words) → Google Translate ──
    if ' ' in word.strip():
        lang = detect_language(raw_text)
        if lang == "uk":
            translation = await google_translate(raw_text, "uk_ru")
            # TTS always Ukrainian — озвучуємо оригінал
            tts_text = raw_text
        else:
            translation = await google_translate(raw_text, "ru_uk")
            # TTS Ukrainian — озвучуємо переклад на укр
        ...
        if translation:
            ...
            text = f"Не вдалося перекласти. Спробуйте ще раз."
            keyboard = []
    ...
    # 1. Try Ukrainian dictionary
    if lang in ("uk", "unknown"):
        found_word, entry = fuzzy_lookup_ukr(word)
        if entry:
            if found_word == word:
                header = f"<b>{word}</b> [ {entry['pos']} ]"
            else:
                header = f"<b>{word}</b> → <b>{found_word}</b> [ {entry['pos']} ]"
            text = f"{header}\n\n{entry['translation']}\n"
            if entry.get("example"):
                text += f"\n<i>{entry['example']}</i>\n"
            ...
            # Verb government
            if found_word in VERB_GOVERNMENT:
                gov = VERB_GOVERNMENT[found_word]
                text += f"\nКерування: {gov['case']} ({gov['question']})\n"

            buttons = build_word_buttons(found_word, entry)
            await update.message.reply_text(
                text, parse_mode=ParseMode.HTML,
    ...
    # 2. Try Russian → Ukrainian reverse dictionary
    if lang in ("ru", "unknown"):
        results = fuzzy_lookup_rus(word)
        if results:
            shown = []
            text = f"<b>{word}</b>\n\n"
            first_ukr = None
            for ukr_word, entry in results[:5]:
                if ukr_word in shown:
                    continue
                shown.append(ukr_word)
                if not first_ukr:
                    first_ukr = (ukr_word, entry)
                text += f"<b>{ukr_word}</b> [ {entry['pos']} ] — {entry['translation']}\n"
                if entry.get("example"):
                    text += f"<i>{entry['example']}</i>\n"
                text += "\n"
            ...
            buttons = []
            if first_ukr:
                buttons = build_word_buttons(first_ukr[0], first_ukr[1])
            else:
                buttons = [[InlineKeyboardButton("В обране", callback_data=f"fav_add_{word}")]]
